...
public class UpdatePhieuXuat extends javax.swing.JDialog {
    ...
    private ArrayList<SP> allProduct;
    ...
    public UpdatePhieuXuat(javax.swing.JInternalFrame parent, javax.swing.JFrame owner, boolean modal) throws UnsupportedLookAndFeelException {
        ...
        // Lay thong tin 
        allProduct = SPDAO.getInstance().selectAllExist();
    ...
    private void loadDataToTableProduct(ArrayList<SP> arrProd) {
        try {
        ...
        try {
            ...
            for (int i = 0; i < CTPhieu.size(); i++) {
                tblNhapHangmd.addRow(new Object[]{
                    i + 1, CTPhieu.get(i).getmaSP(), findSP(CTPhieu.get(i).getmaSP()).gettenSP(), CTPhieu.get(i).getSoLuong(), formatter.format(CTPhieu.get(i).getDonGia()) + "đ"
                });
            }
        } catch (Exception e) {
        }
    ...
    private void btnNhapHangActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnNhapHangActionPerformed
        ...
        if (CTPhieu.isEmpty()) {
            ...
            for (var ct : CTPhieuOld) {
                SPDAO.getInstance().updateSoLuong(ct.getmaSP(), SPDAO.getInstance().selectById(ct.getmaSP()).getSoLuong() + ct.getSoLuong());
        ...
        if (i_row == -1) {
            ...
            if (newSL != null) {
                ...
                try {
                    ...
                    if (soLuong > findSP(CTPhieu.get(i_row).getmaSP()).getSoLuong()) {
        ...
        if (i_row == -1) {
            ...
            if (soluongselect == 0) {
                ...
                if (soluongselect < soluong) {
                    ...
                    if (mtl != null) {
                        if (findSP((String) tblSanPham.getValueAt(i_row, 0)).getSoLuong() < mtl.getSoLuong() + soluong) {
                            JOptionPane.showMessageDialog(this, "Số lượng máy không đủ !");
                        } else {
                            mtl.setSoLuong(mtl.getSoLuong() + soluong);
                        }
                    } else {
                        SP mt = SearchProduct.getInstance().searchId((String) tblSanPham.getValueAt(i_row, 0));
    ...
    private void txtSearchKeyReleased(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_txtSearchKeyReleased
        ...
        ArrayList<SP> Mtkq = new ArrayList<>();
        for (SP i : allProduct) {
